...
DO I=1,INEEDT
	...
	IEXSOLHIR(I)%PROCID=IRECEXR(I)%PROCID
 	...
 	IEXCORDR(I)%NODECORD(1:IRECEXR(I)%MUCHINEED(1),i_cnt4,i_cnt3)=-tolbig
		...
		ALLOCATE(IEXSOLHIR(I)%SOL_huang(IRECEXR(I)%MUCHINEED(1),nof_variables+turbulenceequations+passivescalar,idegfree+1))
		ALLOCATE(IEXSOLHIR(I)%CORD_huang(IRECEXR(I)%MUCHINEED(1),3))
		ALLOCATE(IEXSOLHIR(I)%vol_huang(IRECEXR(I)%MUCHINEED(1),nof_variables+turbulenceequations+passivescalar))
...
DO I=1,TNEEDT
	...
	ALLOCATE (IEXSOLHIS(I)%SOL_huang(IRECEXS(I)%MUCHTHEYNEED(1),nof_variables+turbulenceequations+passivescalar,idegfree+1))
	ALLOCATE (IEXSOLHIS(I)%CORD_huang(IRECEXS(I)%MUCHTHEYNEED(1),3))
	ALLOCATE (IEXSOLHIS(I)%vol_huang(IRECEXS(I)%MUCHTHEYNEED(1),nof_variables+turbulenceequations+passivescalar))
...
DO I=1,INDL
    ...
    ! 	ALLOCATE(IEXBOUNDHIRR(I)%vertpp(IEXCHANGER(I)%MUCHINEED(1),i_cnt2))
        ...
        IF (DG == 1)THEN
			...
			ALLOCATE(IEXBOUNDHIR(I)%FACESOL_DG_huang(IEXCHANGER(I)%MUCHINEED(1),NOF_VARIABLES,number_of_dog+1))
			ALLOCATE(IEXBOUNDHIR(I)%CORD_huang(IEXCHANGER(I)%MUCHINEED(1),3))
        END IF
	 ...
	 I_CNT=(nof_variables+TURBULENCEEQUATIONS+PASSIVESCALAR)+((4+TURBULENCEEQUATIONS+PASSIVESCALAR)*3)
	  ...
	  I_CNT=(nof_variables+TURBULENCEEQUATIONS+PASSIVESCALAR)+((3+TURBULENCEEQUATIONS+PASSIVESCALAR)*2)
        ...
        IF (DG == 1)THEN
			...
			ALLOCATE(IEXBOUNDHIR(I)%FACESOL_DG_huang(IEXCHANGER(I)%MUCHINEED(1),I_CNT,number_of_dog+1))
			ALLOCATE(IEXBOUNDHIR(I)%CORD_huang(IEXCHANGER(I)%MUCHINEED(1),3))
        END IF
...
DO I=1,TNDL
	IF (ITESTCASE.Le.3)THEN
        ...
        IF (DG == 1)THEN
			...
			ALLOCATE(IEXBOUNDHIS(I)%FACESOL_DG_huang(IEXCHANGEs(I)%MUCHTHEYNEED(1),NOF_VARIABLES,number_of_dog+1))
			ALLOCATE(IEXBOUNDHIS(I)%CORD_huang(IEXCHANGEs(I)%MUCHTHEYNEED(1),3))
        END IF
	...
	Else
	 ...
	 I_CNT=(nof_variables+TURBULENCEEQUATIONS+PASSIVESCALAR)+((4+TURBULENCEEQUATIONS+PASSIVESCALAR)*3)
	  ...
	  I_CNT=(nof_variables+TURBULENCEEQUATIONS+PASSIVESCALAR)+((3+TURBULENCEEQUATIONS+PASSIVESCALAR)*2)
	   ...
	   IF (DG == 1)THEN
            ALLOCATE(IEXBOUNDHIS(I)%FACESOL_DG_huang(IEXCHANGEs(I)%MUCHTHEYNEED(1),I_CNT,number_of_dog+1))
			ALLOCATE(IEXBOUNDHIS(I)%CORD_huang(IEXCHANGEs(I)%MUCHTHEYNEED(1),3))
        END IF
