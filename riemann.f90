...
! 			WRITE(190+N,*)FR
			...
			HLLCFLUX(:)=0.5d0*(FL(:)+FR(:))-0.5d0*MAX(ABS(SL(1)),ABS(SR(1)))*(Cright_ROT(:)-Cleft_ROT(:))
			! HLLCFLUX(:)=0.5d0*(FL(:)+FR(:))-0.5d0*min(MAX(ABS(SL(1)),ABS(SR(1)))*1.5d0,speed_HUANG)*(Cright_ROT(:)-Cleft_ROT(:))
			! HLLCFLUX(:)=0.5d0*(FL(:)+FR(:))-0.5d0*speed_huang*(Cright_ROT(:)-Cleft_ROT(:))
...
Subroutine glf_RIEMANN_SOLVER2d(N,CLEFT_ROT,CRIGHT_ROT,HLLCFLUX,ROTVL,ROTVR,GAMMA,SL,SR,SM)
	!> @brief
	!> Rusanov Riemann solver in 2D
		IMPLICIT NONE
		INTEGER,INTENT(IN)::N
	...
	! 			WRITE(190+N,*)FR
				...
				! HLLCFLUX(:)=0.5d0*(FL(:)+FR(:))-0.5d0*MAX(ABS(SL(1)),ABS(SR(1)))*(Cright_ROT(:)-Cleft_ROT(:))
				HLLCFLUX(:)=0.5d0*(FL(:)+FR(:))-0.5d0*speed_huang*1.5d0*(Cright_ROT(:)-Cleft_ROT(:))
				! WRITE(*,*)"MAX(ABS(SL(1)),ABS(SR(1)))="
				! WRITE(*,*)MAX(ABS(SL(1)),ABS(SR(1)))
				! WRITE(*,*)"SPEED_HUANG="
				! WRITE(*,*)speed_huang
	
	END SUBROUTINE glf_RIEMANN_SOLVER2d
