...
INTEGER::I
! CALL EXCHANGE_LOWER(N)
CALL EXCHANGE_HIGHER_huang(N)
CALL EXCHANGE_HIGHER_huang_vol(N)
CALL EXCHANGE_HIGHER_huang_cord(N)

!  CALL TENO_limiter2d(N)
  IF (FASTEST.EQ.1) THEN
      CALL EXCHANGE_LOWER(N)
  ELSE
      CALL EXCHANGE_HIGHER(N)
  ...
  CALL EXHBOUNDHIGHER(N)
  call EXHBOUNDHIGHER_dg(N)
  call EXHBOUNDHIGHER_dg_huang(N)
  ! call EXHBOUNDHIGHER_dg_huang_xxc(N)
  ! CALL TROUBLE_INDICATOR1
  ! Call KXRCF(N)

CALL TENO_indicator2d(N)
! CALL TENO_limiter2dwithdecom(N) !weno

Call TENO_limitermay30(N)

!  call CHECKSOL2Dhuang(N)
...
DO
    ...
    IF (DG.EQ.1)THEN
        ...
        ielem(n,i)%condition=0
        IELEM(N,I)%TROUBLED=0
        IELEM(N,I)%TROUBLED_HUANG=0 ! modified by huang
        END DO
    END IF
...
! Increment time
    ...
    END IF
          ...
          IF (DG.EQ.1)THEN
          ! IF (CODE_PROFILE.ne.102)THEN  !modified by huang
          IF ( mod(it, 100) .eq. 0) THEN
            CALL TROUBLED_HISTORY
          END IF
          ! end if
