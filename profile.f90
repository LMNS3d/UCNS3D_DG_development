...
IF (INITCOND .EQ. 26) THEN !TAYLOR GREEN INITIAL PROFILE

  if (sqrt(((pox(1) - 0.0d0)**2) + ((poy(1) - 0.0d0)**2)) .LE. 0.5d0) then
  ...
  IF (INITCOND .EQ. 106) THEN    !sod
    if (poy(1) .lt. 0.0d0) then
...
IF (INITCOND .EQ. 112) THEN    !single material triple point

if (pox(1) .lt. 1.0d0) then
r1 = 1.0d0
u1 = zero
v1 = zero
p1 = 1.0d0
else
if (poy(1) .gt. 1.5d0 .or. poy(1) .lt. -1.5d0) then
r1 = 0.125d0
u1 = zero
v1 = zero
p1 = 0.1d0
...
IF (INITCOND .EQ. 113) THEN    !scram-jet
