...
SUBROUTINE TROUBLED_HISTORY !modified by huang

INTEGER::I,J,K,TRAJ1,TRAJ2,TRAJ3,TRAJ4,kmaxe,writeid,writeconf,i_huang,ii_huang
REAL::WIN1,WIN2,WIN3,WIN4,POST,POST1,POST2,POST3,POST4
real,dimension(1)::pos_l,pos_g
real,allocatable::x_huang(:), y_huang(:),z_huang(:)
real,dimension(1)::pos_l_huang,pos_g_huang
allocate(x_huang(XMPIELRANK(N)))
allocate(y_huang(XMPIELRANK(N)))
allocate(z_huang(XMPIELRANK(N)))
KMAXE=XMPIELRANK(N)
POST1=0
traj1=0
pos_l(1)=zero
...
pos_g_huang(1)=zero
DO I=1,KMAXE
    pos_l_huang(1)=pos_l_huang(1)+IELEM(N,I)%TROUBLED_HUANG
END DO


CALL MPI_ALLREDUCE(pos_l_huang(1),pos_g_huang(1),1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERROR)

IF (n.eq.0)THEN

OPEN(70,FILE='TROUBLEDcells.DAT',FORM='FORMATTED',ACTION='WRITE',POSITION='APPEND')
WRITE(70,'(E14.7,1X,E14.7,1X,E14.7)')T,(pos_g_huang(1)/IMAXE)
close(70)
...
i_huang = 0
Do I = 1,kmaxe
	if(ielem(n,i)%TROUBLED_HUANG .eq. 1) then
		i_huang = i_huang + 1
		x_huang(i_huang) = ielem(n,i)%xxc
		y_huang(i_huang) = ielem(n,i)%yyc
		z_huang(i_huang) = ielem(n,i)%zzc
	end if
...
IF (n.eq.0)THEN
	...
	OPEN(29,FILE='TROUBLEDcelldetails.DAT',FORM='UNFORMATTED')
	close(29,status='delete')
	
END IF
CALL MPI_BARRIER(MPI_COMM_WORLD,IERROR)
Do ii_huang = 0,isize
	if(N.eq.ii_huang) then
		OPEN(70,FILE='TROUBLEDcelldetails.DAT',FORM='FORMATTED',ACTION='WRITE',POSITION='append')
		do I = 1,i_huang
		WRITE(70,'(E14.7,1X,E14.7,1X,E14.7)')x_huang(i),y_huang(i),IELEM(N,I)%TROUBLED_HUANG
		end do
		close(70)
	end if
	CALL MPI_BARRIER(MPI_COMM_WORLD,IERROR)
