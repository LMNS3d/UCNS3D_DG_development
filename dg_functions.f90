...
FUNCTION DG_SOLFACE_huang(N)
  ...
  INTEGER::I_DOF, I_VAR
  INTEGER,INTENT(IN)::N
  REAL,DIMENSION(1:nof_Variables)::DG_SOLFACE_huang
  REAL,external:: ddot
  ...
  DO I_VAR = 1, NOF_VARIABLES
      DG_SOLFACE_huang(I_VAR) = U_C(ICONSIDERED)%VALDG(1,I_VAR,1) + DDOT(NUMBER_OF_DOG,BASIS_TEMP(1:NUMBER_OF_DOG),1,U_C(ICONSIDERED)%VALDG(1,I_VAR,2:NUMBER_OF_DOG+1),1)
  END DO
...
IMPLICIT NONE
INTEGER,INTENT(IN)::N
INTEGER::I_FACE, I_ELEM, I_QP,iqp,i,KMAXE
REAL::CX,P_huang,OODENSITY

real,dimension(nof_Variables)::leftv,TEMPS
real,dimension(1)::dummyout_huang
real,dimension(1)::dummyin_huang
KMAXE=XMPIELRANK(N)
DO I=1,KMAXE
        
        
  LEFTV(1:NOF_vARIABLES)=U_C(I)%VALdg(1,1:NOF_vARIABLES,1)
...
LEFTV(1:nof_Variables)=TEMPS(1:nof_Variables)
  ...
  cx=MAX(MAX(ABS(LEFTV(2)),ABS(LEFTV(3)))+SQRT(LEFTV(4)*GAMMA/LEFTV(1)),CX)
  ! if(cx.gt.60.0d0)then
  ! write(*,*)"leftv(2)="
  ! write(*,*)leftv(2)
  !   write(*,*)"leftv(3)="
  ! write(*,*)leftv(3)
...
dummyin_huang(1)=0.0d0


CALL MPI_ALLREDUCE(dummyout_huang,dummyin_huang,1,MPI_DOUBLE_PRECISION,MPI_MAX,MPI_COMM_WORLD,IERROR)

speed_HUANG=dummyin_huang(1)
