...
DO I=1,INEEDT
	...
	ALLOCATE (SOLCHANGER(I)%SOL_huang(IEXCHANGER(I)%MUCHINEED(1),nof_variables+turbulenceequations+passivescalar,1:idegfree+1))
	SOLCHANGER(I)%SOL_huang(:,:,:)=0.0d0

	ALLOCATE (SOLCHANGER(I)%vol_huang(IEXCHANGER(I)%MUCHINEED(1),nof_variables+turbulenceequations+passivescalar))
	SOLCHANGER(I)%vol_huang(:,:)=0.0d0

	ALLOCATE (SOLCHANGER(I)%CORD_huang(IEXCHANGER(I)%MUCHINEED(1),3))
	SOLCHANGER(I)%CORD_huang(:,:)=0.0d0
END DO
DO I=1,TNEEDT
	...
	ALLOCATE (SOLCHANGES(I)%SOL_huang(IEXCHANGES(I)%MUCHTHEYNEED(1),nof_variables+turbulenceequations+passivescalar,1:idegfree+1))
	SOLCHANGES(I)%SOL_huang(:,:,:)=0.0d0

	ALLOCATE (SOLCHANGES(I)%vol_huang(IEXCHANGES(I)%MUCHTHEYNEED(1),nof_variables+turbulenceequations+passivescalar))
	SOLCHANGER(I)%vol_huang(:,:)=0.0d0

	ALLOCATE (SOLCHANGES(I)%CORD_huang(IEXCHANGES(I)%MUCHTHEYNEED(1),3))
	SOLCHANGES(I)%CORD_huang(:,:)=0.0d0
END DO
...
DO I=1,TNEEDT
	DO K=1,IEXCHANGES(I)%MUCHTHEYNEED(1)
		  ...
		  SOLCHANGES(I)%SOL_huang(K,1:NOF_VARIABLES,1:IDEGFREE+1)=U_C(IEXCHANGES(I)%LOCALREF(K))%VALdg(1,1:NOF_VARIABLES,1:IDEGFREE+1)
...
IEXCHANGER(I)%MUCHINEED(1)*ITEST,MPI_DOUBLE_PRECISION,&
		...
		CALL MPI_SENDRECV(SOLCHANGES(K)%SOL_huang(1:IEXCHANGES(K)%MUCHTHEYNEED(1),1:ITEST,1:IDEGFREE+1),&
IEXCHANGES(K)%MUCHTHEYNEED(1)*ITEST*(IDEGFREE+1),MPI_DOUBLE_PRECISION,SOLCHANGER(I)%PROCID,&
		SOLCHANGES(K)%PROCID,SOLCHANGER(I)%SOL_huang(1:IEXCHANGER(I)%MUCHINEED(1),1:ITEST,1:IDEGFREE+1),&
IEXCHANGER(I)%MUCHINEED(1)*ITEST*(IDEGFREE+1),MPI_DOUBLE_PRECISION,&
		SOLCHANGER(I)%PROCID,ICPUID,MPI_COMM_WORLD,STATUS,IERROR)
		END IF
		END DO
END DO
...
SUBROUTINE EXCHANGE_HIGHER_huang(N)
!> @brief
!> This subroutine is exchanging the variables of all the halo cells for all the reconstruction stencils
IMPLICIT NONE
INTEGER,INTENT(IN)::N
...
DO I=1,TNEEDT
	DO K=1,IRECEXS(I)%MUCHTHEYNEED(1)
		  ...
		  IEXSOLHIS(I)%SOL_huang(K,1:nof_Variables,1:idegfree+1)=U_C(IRECEXS(I)%LOCALREF(K))%VALDG(1,1:nof_Variables,1:idegfree+1)
	END DO
END DO
!$OMP END DO
...
DO I=1,TNEEDT
	DO K=1,IRECEXS(I)%MUCHTHEYNEED(1)
		  ...
		  IEXSOLHIS(I)%SOL_huang(K,1:nof_Variables+TURBULENCEEQUATIONS+PASSIVESCALAR,1:idegfree+1)=U_C(IRECEXS(I)%LOCALREF(K))%VALDG(1,1:nof_Variables+TURBULENCEEQUATIONS+PASSIVESCALAR,1:idegfree+1)
	END DO
END DO
!$OMP END DO
end if
...
n_requests = n_requests + 1         
      ...
      CALL MPI_IRECV(                                                     &
      IEXSOLHIR(IAVC)%SOL_huang(1:IRECEXR(IAVC)%MUCHINEED(1),1:ITEST,1:idegfree+1),    & !recvbuf
      IRECEXR(iavc)%MUCHINEED(1)*ITEST*(idegfree+1), MPI_DOUBLE_PRECISION,          & !recvcount, recvtype
     IEXSOLHIR(IAVC)%PROCID, 1,                                        & !source, tag
      MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
   )
        END IF
...
!         
		CALL MPI_ISEND(                                                     &
			IEXSOLHIS(IAVT)%SOL_huang(1:IRECEXS(IAVT)%MUCHTHEYNEED(1),1:ITEST,1:idegfree+1), & !sendbuf
			IRECEXS(IAVT)%MUCHTHEYNEED(1)*ITEST*(idegfree+1), MPI_DOUBLE_PRECISION,       & !sendcount, sendtype
			IEXSOLHIS(IAVT)%PROCID, 1,                                        & !destination, tag
			MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
		)
...
!$OMP END MASTER 
!$OMP BARRIER
END SUBROUTINE EXCHANGE_HIGHER_huang


SUBROUTINE EXCHANGE_HIGHER_huang_vol(N)
	!> @brief
	!> This subroutine is exchanging the variables of all the halo cells for all the reconstruction stencils
	IMPLICIT NONE
	INTEGER,INTENT(IN)::N
	...
	DO I=1,TNEEDT
		DO K=1,IRECEXS(I)%MUCHTHEYNEED(1)			  
			  IEXSOLHIS(I)%vol_huang(K,1:nof_Variables)=Ielem(n,IRECEXS(I)%LOCALREF(K))%TOTVOLUME
		END DO
	END DO
	!$OMP END DO
	...
	DO I=1,TNEEDT
		DO K=1,IRECEXS(I)%MUCHTHEYNEED(1)			  
			  IEXSOLHIS(I)%vol_huang(K,1:nof_Variables+TURBULENCEEQUATIONS+PASSIVESCALAR)=ielem(n,IRECEXS(I)%LOCALREF(K))%TOTVOLUME		
		END DO
	END DO
	!$OMP END DO
	end if
	...
	n_requests = n_requests + 1         
		  ...
		  CALL MPI_IRECV(                                                     &
		  IEXSOLHIR(IAVC)%vol_huang(1:IRECEXR(IAVC)%MUCHINEED(1),1:ITEST),    & !recvbuf
		  IRECEXR(iavc)%MUCHINEED(1)*ITEST, MPI_DOUBLE_PRECISION,          & !recvcount, recvtype
		 IEXSOLHIR(IAVC)%PROCID, 1,                                        & !source, tag
		  MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
	   )
	...
	!         
			CALL MPI_ISEND(                                                     &
				IEXSOLHIS(IAVT)%vol_huang(1:IRECEXS(IAVT)%MUCHTHEYNEED(1),1:ITEST), & !sendbuf
				IRECEXS(IAVT)%MUCHTHEYNEED(1)*ITEST, MPI_DOUBLE_PRECISION,       & !sendcount, sendtype
				IEXSOLHIS(IAVT)%PROCID, 1,                                        & !destination, tag
				MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
			)
	...
	!$OMP END MASTER 
	!$OMP BARRIER
END SUBROUTINE EXCHANGE_HIGHER_huang_vol
	


SUBROUTINE EXCHANGE_HIGHER_huang_cord(N)
	!> @brief
	!> This subroutine is exchanging the variables of all the halo cells for all the reconstruction stencils
	IMPLICIT NONE
	INTEGER,INTENT(IN)::N
	...
	DO I=1,TNEEDT
		DO K=1,IRECEXS(I)%MUCHTHEYNEED(1)			  
			  IEXSOLHIS(I)%CORD_huang(K,1)=Ielem(n,IRECEXS(I)%LOCALREF(K))%xxc
			  IEXSOLHIS(I)%CORD_huang(K,2)=Ielem(n,IRECEXS(I)%LOCALREF(K))%yyc
			  IEXSOLHIS(I)%CORD_huang(K,3)=Ielem(n,IRECEXS(I)%LOCALREF(K))%zzc
		END DO
	END DO
	!$OMP END DO
	...
	n_requests = n_requests + 1         
		  ...
		  CALL MPI_IRECV(                                                     &
		  IEXSOLHIR(IAVC)%CORD_huang(1:IRECEXR(IAVC)%MUCHINEED(1),:),    & !recvbuf
		  IRECEXR(iavc)%MUCHINEED(1)*3, MPI_DOUBLE_PRECISION,          & !recvcount, recvtype
		 IEXSOLHIR(IAVC)%PROCID, 1,                                        & !source, tag
		  MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
	   )
			END IF
	...
	!         
			CALL MPI_ISEND(                                                     &
				IEXSOLHIS(IAVT)%CORD_huang(1:IRECEXS(IAVT)%MUCHTHEYNEED(1),1:3), & !sendbuf
				IRECEXS(IAVT)%MUCHTHEYNEED(1)*3, MPI_DOUBLE_PRECISION,       & !sendcount, sendtype
				IEXSOLHIS(IAVT)%PROCID, 1,                                        & !destination, tag
				MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
			)
	...
	!          
	   ...
	   CALL MPI_IRECV(                                                     &
		  IEXSOLHIR(IAVC)%CORD_huang(1:IRECEXR(IAVC)%MUCHINEED(1),1:3),    & !recvbuf
		  IRECEXR(iavc)%MUCHINEED(1)*3, MPI_DOUBLE_PRECISION,          & !recvcount, recvtype
		 IEXSOLHIR(IAVC)%PROCID, 1,                                        & !source, tag
		  MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
	   )        
	...
	!$OMP END MASTER 
	!$OMP BARRIER
END SUBROUTINE EXCHANGE_HIGHER_huang_cord
...
SUBROUTINE EXHBOUNDHIGHER_dg_huang(N)
	!> @brief
	!> This subroutine is communicating the boundary extrapolated values for the variables and their gradients
	!> for the Gaussian quadrature points of direct-side neighbours between MPI processes
	IMPLICIT NONE
	...
	!$OMP DO SCHEDULE (STATIC)
		DO I=1,TNDL
			DO K=1,IEXCHANGES(I)%MUCHTHEYNEED(1)
					...
					IEXBOUNDHIS(I)%FACESOL_DG_huang(K,1:NOF_VARIABLES,1:number_of_dog+1)=u_c(IEXCHANGES(I)%LOCALREF(K))%valdg(1,:,:)
	...
	DO K=1,INDL
	   ...
	   n_requests = n_requests + 1
	   CALL MPI_ISEND(                                                     &
		  IEXBOUNDHIS(J)%FACESOL_DG_huang(1:IEXCHANGES(J)%MUCHTHEYNEED(1),1:I_CNT,1:number_of_dog+1), & !sendbuf
		  IEXCHANGES(J)%MUCHTHEYNEED(1)*I_CNT*(number_of_dog+1), MPI_DOUBLE_PRECISION,       & !sendcount, sendtype
		  IEXBOUNDHIS(J)%PROCID, 0,                                        & !destination, tag
		  MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
	   )
	   ...
	   n_requests = n_requests + 1
	   CALL MPI_IRECV(                                                     &
		  IEXBOUNDHIR(K)%FACESOL_DG_huang(1:IEXCHANGER(K)%MUCHINEED(1),1:I_CNT,1:number_of_dog+1),    & !recvbuf
		  IEXCHANGER(K)%MUCHINEED(1)*I_CNT*(number_of_dog+1), MPI_DOUBLE_PRECISION,          & !recvcount, recvtype
		  IEXBOUNDHIR(K)%PROCID, 0,                                        & !source, tag
		  MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
	   )
...
END SUBROUTINE EXHBOUNDHIGHER_dg_huang
	
SUBROUTINE EXHBOUNDHIGHER_dg_huang_xxc(N)
	!> @brief
	!> This subroutine is communicating the boundary extrapolated values for the variables and their gradients
	!> for the Gaussian quadrature points of direct-side neighbours between MPI processes
	IMPLICIT NONE
	...
	!$OMP DO SCHEDULE (STATIC)
		DO I=1,TNDL
			DO K=1,IEXCHANGES(I)%MUCHTHEYNEED(1)
					...
					! IEXBOUNDHIS(I)%CORD_huang(K,1)=ielem(n,IEXCHANGES(I)%LOCALREF(K))%xxc
					! IEXBOUNDHIS(I)%CORD_huang(K,2)=ielem(n,IEXCHANGES(I)%LOCALREF(K))%yyc
					! IEXBOUNDHIS(I)%CORD_huang(K,3)=ielem(n,IEXCHANGES(I)%LOCALREF(K))%zzc
	...
	DO K=1,INDL
	   ...
	   n_requests = n_requests + 1
	   CALL MPI_ISEND(                                                     &
		  IEXBOUNDHIS(J)%CORD_huang(1:IEXCHANGES(J)%MUCHTHEYNEED(1),3), & !sendbuf
		  IEXCHANGES(J)%MUCHTHEYNEED(1)*3, MPI_DOUBLE_PRECISION,       & !sendcount, sendtype
		  IEXBOUNDHIS(J)%PROCID, 0,                                        & !destination, tag
		  MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
	   )
	   ...
	   n_requests = n_requests + 1
	   CALL MPI_IRECV(                                                     &
		  IEXBOUNDHIR(K)%CORD_huang(1:IEXCHANGER(K)%MUCHINEED(1),3),    & !recvbuf
		  IEXCHANGER(K)%MUCHINEED(1)*3, MPI_DOUBLE_PRECISION,          & !recvcount, recvtype
		  IEXBOUNDHIR(K)%PROCID, 0,                                        & !source, tag
		  MPI_COMM_WORLD, requests(n_requests), ierror                     & !communicator, request handle, error
	   )
...
END SUBROUTINE EXHBOUNDHIGHER_dg_huang_xxc
